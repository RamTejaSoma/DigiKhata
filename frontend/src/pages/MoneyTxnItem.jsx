...
const MoneyTxnItem = () => {
  const { token_exp, backendUrl, items_money, fetchMoneyTxnSubcategories } =
  ...
  const fetchTransactions = async () => {
    try {
      const { data } = await axios.get(
        `${backendUrl}/api/moneyTransaction/subcategory/${item}`,
        {
  ...
  const deleteTransaction = async (id) => {
    try {
      const { data } = await axios.delete(
        `${backendUrl}/api/moneyTransaction/delete/${id}`,
        { headers: { token: token_exp } }
      );
      if (data.success) {
        // fetchSubcategories();
        // fetchTransactions();
        await Promise.all([fetchTransactions(), fetchMoneyTxnSubcategories()]);
  ...
  const submitFilter = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `${backendUrl}/api/moneyTransaction/filter?${params}`,
        {
          headers: { token: token_exp },
        }
      );
  ...
  const fetchMoneyData = async () => {
    ...
    try {
      const params = new URLSearchParams(filtersToSend).toString();
      const { data } = await axios.get(
        `${backendUrl}/api/moneyTransaction/summary?${params}`,
        {
          headers: { token: token_exp },
        }
      );
    ...
    <div>
      <div className="relative flex flex-col sm:flex-row gap-2 mt-2">
        ...
        >
          ...
          {Array.isArray(items_money) && items_money.length > 0 ? (
            items_money.map((item, index) => (
              <p
                key={index}
      ...
      `}
                onClick={() => navigate(`/moneyTransaction/${item}`)}
              >
                {item.name || item}
              </p>
            ))
        ...
        <div className="text-xs sm:text-base flex flex-col gap-4 mt-3 w-full mx-auto bg-violet-100 rounded-lg shadow-lg p-2 overflow-y-auto max-h-[72vh] min-w-[15rem]">
            ...
            transactions.map((txn, index) => (
              ...
              >
                <div className="flex flex-col gap-1 flex-1 min-w-0">
                  <p className="font-bold text-gray-800">{txn.entityName}</p>
